...
def get_model(options: dict, embedding: np.ndarray):
    print('get model')
    classifier = options['classifier']
    if classifier == 'LSTM_01':
        return get_baseline_model(options, embedding)
    ...
    # LSTM_05 variant using Advanced Activation Function
        ...
        # multiplying warrant0 and warrant1 separately with reason and claim - similar to attention
        warrant0 = LSTM(lstm_size, activation='linear')(merger()([al[0], al[2], al[3]]))
        warrant1 = LSTM(lstm_size, activation='linear')(merger()([al[1], al[2], al[3]]))
...
def get_baseline_model(options: dict, embedding: np.ndarray):
    ...
    sequence_layer_debate_title_input = Input(shape=(max_len,), dtype='int32', name="sequence_layer_input_debateTitle")
    sequence_layer_debate_info_input = Input(shape=(max_len,), dtype='int32', name="sequence_layer_input_debateInfo")

    # now define embedded layers of the input
    embedded_layer_warrant0_input = Embedding(
        embedding.shape[0], embedding.shape[1], input_length=max_len, weights=[embedding], mask_zero=True)(
